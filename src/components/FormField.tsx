import React, { useCallback, useRef, memo, FC, useState, Key, useEffect } from 'react'
...
import { classNodeDataSchema } from '../type/zodClassNodeComp.zod'
...
const FuncFormFieldfunc: FC<fieldProps<'data.functions'>> = (props) => {
...
const VarFormFieldfunc: FC<fieldProps<'data.variables'>> = (props) => {
  const { item, index, remove } = props
  const [visibility, setVisibility] = useState<ResponsiveValue<Property.Visibility>>('hidden')
  const [display, setDisplay] = useState<ResponsiveValue<Property.Display>>('none')
  const {
...
const FuncFormField = memo(FuncFormFieldfunc)
const VarFormField = memo(VarFormFieldfunc)
